...
for r in $(git branch -r | grep -v -e '->'); do
...
echo "$MERGED" | grep -q "path" || { echo "❌ path branch not merged"; exit 1; }
echo "$MERGED" | grep -q "truth" || { echo "❌ truth branch not merged"; exit 1; }
